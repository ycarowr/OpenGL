...
void Start()
{
	...
	//getting the address of the uniform u_Color

	GlCall(colorUniformId = glGetUniformLocation(program, "u_Color"));
	ASSERT(colorUniformId != -1);
	
	//set the values of the uniform in the gpu
...
//Change the uniform value over the time
...
void Update()
{
	//uniforms change per draw call. Which means it has to call the draw method in order to take effect
	//and depending the layout data we are using this limitation can be a blocker.
...
int main(void)
{
	...
	//set the interval to 1 to fix the frame rate
