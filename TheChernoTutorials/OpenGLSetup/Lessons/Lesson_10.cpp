...
void Start()
{
	...
	glGenBuffers(amount, &dataBuffer);
	...
	glGenBuffers(amount, &indexBuffer);
	...
	glBufferData(GL_ARRAY_BUFFER, size, triagles, usage);
...
void Update()
{
	...
	GL_ClearError();
	//a draw call!
	GLCALL(glDrawElements(mode, count, GL_INT, nullptr));
}
